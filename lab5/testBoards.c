...
void testBoards() {
  ...
  board6.squares[TOP][MID] = MINIMAX_O_SQUARE;
  board6.squares[TOP][RGT] = MINIMAX_O_SQUARE;
  board6.squares[MID][LFT] = MINIMAX_X_SQUARE;
  ...
  board7.squares[MID][MID] = MINIMAX_X_SQUARE;
  ...
  board11.squares[TOP][RGT] = MINIMAX_EMPTY_SQUARE;
  board11.squares[MID][LFT] = MINIMAX_O_SQUARE;
  board11.squares[MID][MID] = MINIMAX_X_SQUARE;
  ...
  board14.squares[BOT][RGT] = MINIMAX_O_SQUARE;
  ...
  minimax_computeNextMove(&board14, true, &row,
                          &column); // true means X is current player.
  printf("next move for board14: (%d, %d)\n", row, column);
  minimax_computeNextMove(&board15, true, &row,
                          &column); // true means X is current player.
